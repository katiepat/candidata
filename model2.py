...
class Candidate(db.Model):
    ...
    summary = db.relationship('Candidate_Summary', foreign_keys=[cand_summary_id], backref='candidate')
...
class Organization(db.Model):
    ...
    org_summary_id = db.Column(db.Integer, autoincrement=True, nullable=False, primary_key=True)
...
class Candidate_Organization(db.Model):
    ...
    cid = db.Column(db.String(100), db.ForeignKey('candidates.cid'), nullable=False)
    org_id = db.Column(db.Integer, db.ForeignKey('organizations.org_summary_id'), nullable=True)
    org_name = db.Column(db.String(200), nullable=False)
    total = db.Column(db.Float(), nullable=False)
    pacs = db.Column(db.Float(), nullable=True)
    individuals = db.Column(db.Float(), nullable=True)

    orgs = db.relationship('Organization', foreign_keys=[org_id], backref='cand_orgs')
...
def connect_to_db(app):
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql:///candidata2'
